...
DualCorePriority::DualCorePriority(const std::vector<Process>& processes)
    : DualCoreScheduler(processes, 0) {}

DualCoreScheduleResult DualCorePriority::schedule() {
    ...
    while (completedCount < processList.size()) {
        ...
        std::sort(available.begin(), available.end(),
                  [&](int a, int b) {
                      return processList[a].priority < processList[b].priority;
                  });
